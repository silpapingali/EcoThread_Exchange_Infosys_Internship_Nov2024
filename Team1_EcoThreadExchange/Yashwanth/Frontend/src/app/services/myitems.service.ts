...
export class MyItemsService {
  ...
  private apppiUrl = `${environment.apiUrl}/myitems/myitemforpropose`;
  getMyItemsforpropose(filters: any = {},page: number, pageSize: number): Observable<any> {
    const token = localStorage.getItem('token');
    if (!token) {
      return throwError(() => new Error('User is not authenticated'));
    }
    ...
    if (filters.exactMatch !== undefined) params = params.set('exactMatch', filters.exactMatch.toString());
    if (filters.date) params = params.set('date', filters.date);
    return this.http.get<any>(`${this.apppiUrl}?page=${page}&pageSize=${pageSize}`, { headers, params }).pipe(
      catchError((error) => {
        console.error('Error in fetching my items:', error);
        return throwError(() => new Error('Failed to fetch my items'));
      })
